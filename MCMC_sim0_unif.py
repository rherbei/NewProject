#################################
##Simulate s=0, use Uniform prior
#################################
...
###prior density for p###
discretep=np.array(1./99.).repeat(99)
...
for i in range(reps):
    ...
    simqs=np.empty(K)
    for k in range(K):
        simqs[k]=Algorithm2(T,theta,theta,initps[k])
    n=200
...
np.savetxt('Chains/MCMC_sim0_unif_ss4.txt',allss)
#np.savetxt('Chains/MCMC_sim0_unif_qs4.txt',allqs)

#save rejection counts
np.savetxt('Output/MCMC_sim0_unif_rejS4.txt',rejs)
np.savetxt('Output/MCMC_sim0_unif_rejQ4.txt',rejq)
