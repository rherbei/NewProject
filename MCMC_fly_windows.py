###############################################
##MCMC for sliding windows for hypoxic fly data
###############################################
...
###load neutral data###
flyn=np.loadtxt('Data/fly2Ln.txt')
nlocs=np.loadtxt('Data/nlocs2L.txt')

###load data with selection###
flies=np.loadtxt('Data/fly2Ls.txt')
slocs=np.loadtxt('Data/slocs2L.txt')
n=200 
allY=map(round,flies*n)
...
##create array to store chains for selection parameter
schains=np.empty((steps,windowct))

##Loop through moving windows
for i in range(windowct):

    ##take subset of final allele frequencies
    Y=allY[(i*windowshift):(i*windowshift+windowsize)]

    ##find correct region for initial allele frequencies
    initrgn=flyn[np.logical_and(nlocs>=slocs[(i*windowshift],nlocs<=slocs[(i*windowshift+windowsize)])]

    ###empirical density for p###
    pdensity=stats.gaussian_kde(initrgn)
    discretep=np.empty(99)
    for r in range(99):
        discretep[r]=pdensity.evaluate(r/100.)
...
#save the parameter parameter estimates
np.savetxt('Chains/MCMC_fly2L_windows.txt',schains)